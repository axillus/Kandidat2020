...
def main():
    ...
    for gissning in range(3):
        ...
        if gissning == 0:
            ...
            num_iterations_tot = np.array([iterations])
        else:
            history_full = np.append(history_full, history, axis=0)
            num_iterations = np.append(num_iterations, iterations)
            num_iterations_tot = np.append(num_iterations_tot, num_iterations_tot[gissning - 1] + iterations)
    plotta_upp(history_full, num_iterations, num_iterations_tot, constants, data_concentration, data_info, ode_info)
